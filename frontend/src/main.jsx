            ...
            <div className="workspace">
                ...
                <main id="main">
                    ...
                    <button className="reminders-banner" onClick={() => setModal({ kind: "reminders" })}>
                        ...
                        <span>
                            <strong>
                                {upcoming.length ? `${upcoming.length} follow-up${upcoming.length === 1 ? "" : "s"} on your radar` : "Your follow-up reminders"}
                            </strong>
                            <small>
                                {upcoming.length
                                    ? `Next: ${upcoming[0].company} · ${upcoming[0].followUp}`
                                    : "Add a follow-up date in Applications to stay on track."}
                            </small>
                        </span>
                        <span className="reminders-link">
                            View reminders <ArrowRight size={20} />
                        </span>
                    </button>
                    {tab === "Jobs" && (
                        <>
                            ...
                            <section className="jobs-section">
                                <div className="section-heading">
                                    ...
                                    >
                                        ...
                                        <span className="saved-jobs-count" aria-label={`${saved.length} saved jobs`}>
                                            {saved.length}
                                        </span>
                        ...
                        <div>
                            ...
                            <div className="resume-layout structured-resume-layout">
                                <section className="resume-list">
                                    ...
                                    <ReactionMeme
                                        className="resume-meme"
                                        meme={[
                                            "image-folder/06.png",
                                            "Character smiling mischievously",
                                            "me turning the group project into leadership experience",
                                        ]}
                                    />
                                </section>
                    ...
                    {tab === "Inbox" && (
                        <section className="inbox">
                            ...
                            {messages.length ? (
                                    ...
                                    .map((m) => (
                                        <article className="message-card" key={m.id}>
                                            ...
                                            <div className="message-actions">
                                                <button
                                                    className="secondary"
                                                    aria-label={`Edit log for ${m.company}`}
                                                    onClick={() => setModal({ kind: "message", message: m })}
                                                >
                                                    <Pencil size={18} /> Edit
                                                </button>
                                                <button
                                                    className="secondary"
                                                    aria-label={`Delete log for ${m.company}`}
                                                    onClick={() => setModal({ kind: "delete-message", message: m })}
                                                >
                                ...
                                <div className="empty">
                                    <ReactionMeme
                                        className="inbox-waiting-meme"
                                        meme={[
                                            "image-folder/05.png",
                                            "Sad character watching something crumble into dust",
                                            "me waiting for the recruiter to reply",
                                        ]}
                                    />
                ...
                <Modal onClose={() => setModal(null)}>
                    ...
                    {modal.kind === "job" && (
                        <>
                            ...
                            {modal.job.id === 4 && (
                                <ReactionMeme
                                    className="job-detail-meme"
                                    meme={[
                                        "image-folder/09.png",
                                        "Pig-faced character taking a selfie beside coastal cliffs",
                                        "first day at Touch Grass Technologies",
                                    ]}
                                />
                            )}
                            <p className="muted">
                    ...
                    {modal.kind === "message" && (
                        <form
                            onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    if (modal.message) {
                                        const updated = await apiRequest(`/api/communications/${modal.message.id}/`, auth.access, {
                                            method: "PATCH",
                                            body: JSON.stringify(payload),
                                        });
                                        setMessages(messages.map((m) => (m.id === updated.id ? updated : m)));
                                        setNotice("Communication updated.");
                                    } else {
                        ...
                        >
                            ...
                            <div className="form-row">
                                ...
                                <label>
                                    Date
                                    <input name="date" type="date" required defaultValue={modal.message?.date || new Date().toLocaleDateString("en-CA")} />
                                </label>
                            </div>
                            <label>
                                Notes
                                <textarea name="notes" required defaultValue={modal.message?.notes} placeholder="What did they say? What happens next?" />
                            </label>
                            <button className="primary" type="submit">
                    ...
                    {modal.kind === "delete-message" && (
                        <div>
                            <p>
                                Delete the {modal.message.type.toLowerCase()} log for <strong>{modal.message.company}</strong> from {modal.message.date}? This
                                cannot be undone.
                            </p>
                            <div className="message-actions">
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        try {
                                            await apiRequest(`/api/communications/${modal.message.id}/`, auth.access, { method: "DELETE" });
                                            setMessages(messages.filter((m) => m.id !== modal.message.id));
                                            setModal(null);
                                            setNotice("Communication deleted.");
                                        } catch (error) {
                                            setNotice(`Could not delete communication: ${error.message}`);
                                        }
                                    }}
                                >
