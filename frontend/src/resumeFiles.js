...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onblocked = () =>
      reject(new Error("Close other Hire Power tabs and try again."));
  });
}
...
async function transact(mode, action) {
  ...
  return new Promise((resolve, reject) => {
    ...
    transaction.onabort = transaction.onerror = () => {
      db.close();
      reject(
        transaction.error || new Error("Could not access resume storage."),
      );
    };
  });
}

export const listResumeFiles = () =>
  transact("readonly", (store) => store.getAll());
export const removeResumeFile = (id) =>
...
export async function storeResumeFile(file) {
  ...
  if (extension === "pdf" && header !== "%PDF-")
    throw new Error("That file does not appear to be a PDF.");
  if (extension === "docx") {
